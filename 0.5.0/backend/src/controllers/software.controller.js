...
exports.softwareController = {
    ...
    getById: async (req, res, next) => {
        try {
            ...
            const user = req.user;
            const item = await software_service_1.softwareService.getById(id, user);
    ...
    create: async (req, res, next) => {
        try {
            const user = req.user;
            const newItem = await software_service_1.softwareService.create(req.body, user);
    ...
    update: async (req, res, next) => {
        try {
            const id = req.params.id;
            const user = req.user;
            const updatedItem = await software_service_1.softwareService.update(id, req.body, user);
    ...
    delete: async (req, res, next) => {
        try {
            const id = req.params.id;
            const user = req.user;
            await software_service_1.softwareService.delete(id, user);
            res.status(204).send();
        }
        catch (error) {
            next(error);
    ...
    search: async (req, res, next) => {
        try {
            const q = req.query.q || '';
            const items = await software_service_1.softwareService.search(q);
            res.status(200).json({ data: items, meta: { total: items.length } });
        }
        catch (err) {
            next(err);
